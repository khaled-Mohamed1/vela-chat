...
//                'conversations' => $conversations
    ...
    public function infoGroupConversation(Request $request): JsonResponse
    {
        ...
        try {
            ...
            }])->select('id','name','image','type')->find($request->conversation_id);
    ...
    public function infoPeerConversation(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $conversations = Participant::with(['ConversationParticipant' => function($query) use ($request){
                $query->where('type', 'group')->with(['usersConversation' => function($query) use($request){
                    $query->where('user_id', $request->receiver_id);
                }]);
            }])->select('id', 'conversations_id')->where('user_id', $request->user_id)->get();
